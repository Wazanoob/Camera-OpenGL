...
void RendererOGL::drawMeshes()
{
	...
	glDisable(GL_BLEND);
	Shader& shader = AssetsManager::getShader("Phong");
	shader.use();
	...
	for (auto mc : meshes)
	{
		mc->draw(AssetsManager::getShader("Phong"));
	}
}
...
void RendererOGL::drawSprites()
{
	...
	Shader& spriteShader = AssetsManager::getShader("Sprite");
	spriteShader.use();
...
void RendererOGL::drawSprite(const Actor& actor, const Texture& tex, Rectangle srcRect, Vector2 origin, Flip flip) const
{
	...
	AssetsManager::getShader("Sprite").setMatrix4("uWorldTransform", world);
	tex.setActive();
